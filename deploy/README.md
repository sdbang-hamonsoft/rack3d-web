...
> **rack3d 는 회사(한몬소프트) 프로젝트다. 배포 대상은 buru-ext 다.**
>
> ⚠️ 전역 규칙(`~/.claude/CLAUDE.md`)의 "GitHub Actions 대신 **맥미니 원격 빌드**가 표준"은
> **이 프로젝트에 해당하지 않는다.** 그건 개인 프로젝트용 패턴이고, 여기서는 사내 파이프라인
> (buru-ext 의 ansible + 사내 레지스트리 + 사내 k8s)이 이미 있고 그것이 표준이다.
> 맥미니에서 빌드하거나 `scripts/deploy_mini.sh` 같은 것을 찾지 마라 — 없고, 만들 것도 아니다.
...
가리킨다. 그 장비는 **사용자 자택의 개인 프로젝트 서비스 서버**이고 **회사 것이 아니다.**
rack3d 는 회사 프로젝트이므로 **거기 배포하는 일은 절대 없다.**
로컬에서 `kubectl -n rack3d ...` 를 치면 남의(개인) 클러스터를 보게 되고, "파드가 없다"는
엉뚱한 결론이 나온다. k8s 명령은 전부 `ssh buru-ext` 안에서 돈다.
