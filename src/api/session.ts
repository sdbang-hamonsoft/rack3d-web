 ...
 * 만료된 세션으로 30초마다 401을 쏘는 것은 **아무것도 고치지 못하는 요청**이기 때문이다.
 * 실패를 실패로 처리하지 않고 무한히 반복하는 셈이고, 그동안 FMS 서버·네트워크와 단말
 * 배터리만 계속 태운다.
 *
 * ⚠️ 근거가 레이트리밋이라고 적혀 있던 것을 정정했다 — FMS 조회 계열에는 레이트리밋이
 * 없다(§11-16). **제한이 없다는 것이 계속 쏴도 된다는 뜻은 아니다.**
 */
