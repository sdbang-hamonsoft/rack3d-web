...
### 11-35. netis-fms PM — E17 착수 접수: 캐시 정책 파이프라인 준비 + 실사진 계획 확인 중 (2026-08-23)

E17 착수 알림 접수. 약속대로 착수 전 조율.

**?v=<sha> 캐시 정책 — FMS 파이프라인 태운다(§11-15 그대로).** 구현 범위(작음): `AssetImageController.serve`(현재 `CacheControl.maxAge(60s).cachePrivate()`, :76)를
- **`v` 파라미터 있으면** → `Cache-Control: private, max-age=31536000, immutable`(내용은 항상 현재 이미지, v는 캐시 키용·값검증 불요).
- **`v` 없으면(기존 계약)** → 기본 `max-age` **60s→3600s** 상향(ETag 재검증 유지).
- `private` 유지(ASSET READ 인증 콘텐츠). 안정 URL 순수 immutable 금지(교체 후 stale) — 버전 파라미터로 회피, 지적대로.
원본·`?variant=texture` **둘 다** 적용. 제품 오너에게 착수 우선순위 확인 중(내 약속대로) — OK 나오면 구현→리뷰(보안)→QA→배포하고 태그 공유한다. 급하지 않다니(60s로도 동작) rack3d는 `?v=` 부착을 먼저 넣어도 된다.

**dir 규약** §11-30 그대로: FRONT=로컬 +Z 면, REAR=반대. E18에서 노드실측(Rack_Frame_Front z=+0.477)+회전행렬로 확정·코드주석화 — 재확인 불요. 좋다.

**점진 로딩(앞면·근거리 우선, 뒷면 지연)** 방향 합리적이다 — FMS 조회 무제한(§11-16)이라 서버 제약은 없고, 캐시 정책이 붙으면 재요청도 immutable로 0회가 되니 궁합이 좋다.

**실사진 교체 계획 — 제품 오너에게 확인 중.** 현재 이미지는 테스트 합성(가로형, U높이 비례 §11-14)이 맞다. 실장비 정면 사진 교체 여부·시점은 오너 결정이라 지금 묻는다. 계획 정해지면 공유하겠다 — 종횡비(rack3d가 평면 스케일로 흡수)·크롭 범위는 실사진이 올라와야 최종 확인 가능하다는 데 동의(그때 검증 시점 함께 잡자).

### 11-36. rack3d 배포 — E17 장비 실물 사진 텍스처 `main-c050820` (2026-08-23)

**§11-15(캐시)·§11-14(종횡비)·§11-30(dir) 합의대로 구현해 운영 반영했다.** 이제 랙 안 장비의 앞뒤면이 GLB 에 구워진 기본 사진이 아니라 **FMS 에 등록된 자산별 실물 사진**으로 덮인다.

| 배포 | 값 |
|---|---|
| 이미지 | `10.1.20.21:5000/rack3d-web:main-c050820` |
| 롤아웃 | 무중단, 재시작 0 |
| 운영 확인 | `https://fms.burunet.co.kr/rack3d/` 200, 번들에 `?variant=texture` 호출 포함, `Cache-Control` 단일 헤더(`public, max-age=15552000`), index.html `no-cache` |

**FMS 계약을 쓴 방식**
- `GET /api/assets/{id}/images` 로 메타를 먼저 받고 **`hasFront`/`hasRear` 가 false 면 이미지 엔드포인트를 아예 부르지 않는다.** 없는 줄 알면서 404 를 받아내는 요청은 랙 수십 대 규모에서 수백 건의 헛된 왕복이다(실측 0건 확인).
- `GET /api/assets/{id}/images/{FRONT|REAR}?variant=texture&v=<sha256>` — **`v` 는 메타의 `sha256` 그대로만 붙인다. 모르면 안 붙인다.** 지어낸 v 로 1년 캐시에 옛 바이트를 박는 것이 §11-15 가 경고한 바로 그 사고 지점이다. 소문자 64자만 통과시킨다(실응답 실측 기준).
- FMS 가 아직 `max-age=60, private` 로 답하고 있어 **immutable 분기는 서버 쪽 배포를 기다린다** — rack3d 는 `?v=` 부착만 먼저 넣었다(§11-35 에서 그렇게 해도 된다고 확인받았다). FMS 가 올리면 rack3d 변경 없이 재검증이 0 건이 된다.
- 종횡비는 **하드코딩하지 않는다**(§11-14). 지오메트리 bbox 와 이미지 실픽셀을 런타임 실측해 폭만 늘린다(현재 이미지 기준 1.0987배, 445 → 489mm). **높이는 안 건드린다** — U 점유는 실데이터다.
- `dir` = FRONT 방위 규약대로 FRONT 를 로컬 +Z 면에 붙였다. E18 노드 실측 그대로라 재확인 없이 맞았다.

**로딩 정책 — 진입 시 전량 요청하지 않는다.** 앞면 우선·근거리 우선, 거리 히스테리시스 9m/11m, 동시 4건, 활성 48면. 조회 무제한(§11-16)이라도 랙 36대 × 장비 9대 × 2면이면 한 번에 600건이 넘어가고, 그건 서버가 아니라 **브라우저 GPU 가 먼저 죽는다.**

**⚠️ 아직 실사진으로 검증한 것이 아니다.** 현재 FMS 이미지는 테스트 합성물(가로형·우측 여백)이라 "실물처럼 보이는지"는 판단하지 않았다. §11-35 의 실사진 교체 계획이 정해지면 **1~2장만 먼저** 올려 주면 좋겠다 — 아래 건이 그때 드러난다.

**⚠️ FMS 에 미리 알리는 건 — 실사진이 오면 좌우가 잘린다.** rack3d GLB 실측: 장비 사진 평면은 z=377.5mm 인데 마운팅 레일이 z=382.5~407.5mm(x 217~239mm), 앞 포스트가 z=455~500mm 로 **사진보다 앞에 있다.** 종횡비 보정으로 반폭이 222.5 → 244.5mm 로 늘면 늘어난 22mm 중 **16.5mm 가 정확히 레일 x 밴드 안**으로 들어가, 정면에서도 좌우 각 22mm(전폭 4.5%)가 레일 뒤에 가린다. 포스트가 77.5mm 앞이라 **정면에서 7.7°만 벗어나면** 보정분 전체가 포스트에 가린다.

`?variant=texture` 가 **마운팅 이어를 포함한 19인치 패널**이라(§11-14) 잘리는 좌우 22mm 가 바로 **이어·전원 버튼·상태 LED·서비스 태그** 자리다. **이건 FMS 잘못이 아니라 rack3d GLB 쪽 문제다** — 해결은 rack3d 가 한다(사진 평면을 레일 앞으로 옮기거나 letterbox 로 바꾸는 것 중 실사진 보고 정한다). FMS 에 부탁할 것은 **크롭 규격을 바꾸는 게 아니라 실사진 1~2장**뿐이다. 지금 규격을 바꾸면 잘못된 쪽에 맞추게 된다.

### 11-37. 사진 교체가 화면에 반영되지 않는다는 신고 — 오진 기록 (2026-08-23)

**결론부터: 버그가 아니었다. 교체한 자산과 화면에서 보던 자산이 달랐다.** 남기는 이유는 rack3d 가
잘못된 진단을 FMS 에 보냈고, 그 근거로 FMS 가 실제 코드를 고쳐 배포(`main-d290bcf`)했기 때문이다.

**신고**: FMS 환경설정에서 앞/뒤 이미지를 교체했는데 화면이 그대로다. 강제 새로고침도 소용없다.
같은 자산이 rack3d 에서는 바뀌었다.

**rack3d 가 처음 내린 진단(틀렸다)**: FMS 편집 화면이 `?v` 없는 고정 URL 로 이미지를 받는데
(`frontend/src/api/rackMapping.ts:88`) `main-b8bc839` 가 그 분기의 `max-age` 를 60 → 3600 초로
올렸으니, 프레시니스 1시간 동안 재검증이 막혀 옛 사진이 남는다 — §11-15 가 경고한 함정이
FMS 자기 화면에서 재현된 것이다.

**실측(UAT 조회)**

| 자산 | 위치 | FRONT | REAR | uploadedAt |
|---|---|---|---|---|
| 5 (AST-2026-S014) | 랙 A-01 | 30,177 B jpeg | 54,360 B jpeg | **2026-08-23 02:38 / 02:39** |
| 7 (AST-2026-S020) | 랙 **A-02** ← 스크린샷 | 869 B png | 867 B png | 2026-08-22 03:41 |

**사용자가 교체한 것은 자산 5, 화면에서 보고 있던 것은 자산 7이다.** 자산 7 은 어제 초기 데이터
그대로라 새로고침해도 바뀔 것이 없었다. 업로드도 서버 저장도 FMS 화면도 전부 정상이었고,
rack3d 에서 바뀐 것도 같은 이유(자산 5)다.

**무엇을 잘못했나**: 스크린샷에 **자산 ID 가 찍혀 있었는데 읽지 않고** 캐시 헤더 코드만 보고
단정했다. 코드가 그럴듯하면 증상도 그것이라고 믿어 버린 것이다. 관측과 설명을 맞춰 보는 절차가
빠졌다 — "이 설명이 맞다면 서버의 바이트는 무엇이어야 하는가"를 먼저 확인했으면 3분이면 갈렸다.

**그럼에도 `main-d290bcf` 는 유지하도록 요청했다.** 고정 URL + `max-age` 3600 이면 실제 교체 시
stale 이 나는 것은 코드상 사실이고, 편집 모달은 저트래픽이라 매 로드 조건부 요청(`no-cache`)
비용이 무의미하다. **다만 근거는 "관측된 회귀"가 아니라 "미관측 잠재 위험 — 선제 차단"이다.**
FMS 커밋 메시지의 "교체 후 옛 사진이 최대 1h 남음"은 관측된 적이 없으므로 그렇게 고쳐 달라고 했다.

**부수 확인(rack3d 는 이 부류에 노출되지 않는다)**: 메타 JSON(`GET /assets/{id}/images`)이
`no-cache, no-store, must-revalidate` 로 내려온다. sha 가 브라우저 캐시에 굳지 않으므로 사진이
교체되면 다음 메타 조회에서 새 sha → 새 URL → 새 바이트로 반드시 넘어간다. rack3d 쪽 최대 지연은
자체 메모리 메타 캐시 **5분**(`assetPhotos.ts` `META_TTL_MS`)이고, 이미 붙은 면은 수요가 유지되는
동안 재확인하지 않으므로 **랙 바로 앞에 카메라를 세워 둔 채로는 즉시 반영되지 않는다**
(멀어졌다 돌아오거나 새로고침하면 반영). 관제 화면 성격상 결함으로 보지 않으나 사용자에게 밝혔다.

### 11-38. rack3d → netis-fms 협의 요청: 배치 오브젝트 3D 모델 3건 (2026-09-03)

배치 오브젝트를 색 박스에서 실사풍 3D 모델로 바꾸는 작업을 시작했다(제품 오너 지시,
근거·규약은 `docs/layout-object-modeling-plan.md`). 현재 4종을 만들었고 씬 연결까지 마쳤다.
진행하면서 **FMS 결정이 필요한 것 3건**이 나왔다.

#### ① `GAS` 는 가스 *감지기* 가 맞나 — 소화 설비 타입이 필요하다

제품 오너가 준 3D 사양서에 "가스 소화 설비(Gas Fire Suppression)"가 있어 `GAS` 에 소화 용기
뱅크 모델을 붙였는데, **실 ZONE 화면에서 잡았다**(A구역 서버랙 `LOC-A-B1-ZA`). 그 화면의
`GAS` 오브젝트 레이블이 "가스감지"였고, §11-31 ① 팔레트 정의도 `가스감지 / 가스 감지 / #F57C00`다.
`FIRE`(화재감지)·`WATER`(누수감지)·`SEISMIC`(지진감지)와 나란한 **감지기 계열**로 보인다.

→ rack3d 는 `GAS` 매핑을 **되돌렸다**. `GAS` 는 지금처럼 낮은 색 박스(0.3m)로 그린다.

**요청**: ⓐ `GAS` = 가스 감지기가 맞는지 확인. ⓑ 맞다면 **가스 소화 설비용 type 신설**.
현 팔레트 12종에 소화 설비에 해당하는 것이 없다. 이름은 FMS 가 정하면 rack3d 가 따른다
(rack3d 코드에는 `SUPPRESSION` 으로 가정해 두었고, 다른 이름이면 한 줄 교체다).

#### ② 배터리 랙 type 신설 (`BATTERY`)

같은 사양서의 "배터리 랙(Battery Rack)"도 팔레트 12종에 대응이 없다.
`UPS`·`POWER` 로 대신 놓으면 2D 에디터에서도 구분이 안 된다.

**요청**: `type` 팔레트에 배터리 랙 추가 + 2D 에디터 색 배정.
색은 기존 12색과 구분되고 `UPS`(#7B1FA2)·`POWER`(#C2185B)와 헷갈리지 않는 값이면 된다.
기존 데이터 마이그레이션은 불필요하다(신규 배치부터).

rack3d 는 두 건 모두 **모델을 미리 만들어 두고 기다린다.** `LayoutObject.type` 이 유니온이 아니라
`string` 이라, FMS 가 새 type 을 서빙하기 시작하면 rack3d 를 고치지 않아도 모델이 붙는다.

#### ③ 다중칸 점유 — §11-31 ② 재검토 요청

§11-31 ②에서 "전 오브젝트를 tileMm 1칸 크기로 그리면 된다, 실제 항온항습기가 600mm보다 커도
1칸 단순화"로 정리했다. **실사 모델을 넣으니 이 단순화가 깨진다.**

| 오브젝트 | 실측 근거 | 타일(600mm) 환산 |
|---|---|---|
| 항온항습기 | STULZ CyberAir 3 PRO DX 1200 x 890mm (제품 오너 제공 사양) | 2.00 x 1.49 칸 |
| 가스 소화 설비 | 용기 4본 뱅크 1060 x 320mm | 1.77 x 0.53 칸 |
| UPS | 600 x 850mm (실측 대기, 중형 타워 근사) | 1.00 x 1.42 칸 |

1칸(504mm)에 맞추려고 폭을 줄이면 비율이 무너져 **사물함처럼 보이고**, 비율을 지킨 채 축소하면
항온항습기 높이가 1.98m → 0.83m 가 되어 2m 랙 옆에서 알아볼 수 없다.

rack3d 는 일단 **실치수로 그리고 칸을 넘기기로** 했다. 근거는 **이미 그렇게 하고 있다는 것**이다 —
`rack-42u.glb` 가 깊이 1.0m 로 **1.67칸**을 쓰고 있고(실측), 랙은 진작부터 앞뒤 타일을 넘어 서 있었다.

⚠️ **부작용**: 옆 칸에 오브젝트가 있으면 3D 에서 겹쳐 보인다. `UNIQUE(zone,x,z)` 상 FMS 는
한 칸에 하나만 두므로, **넓은 오브젝트 옆 칸을 비워두지 않으면** 관제 화면에서 형상이 파고든다.

**요청**: ⓐ 실배치도에서 항온항습기·소화 설비 옆 칸에 다른 오브젝트를 두는 사례가 있는지 확인.
ⓑ 있다면 `zone_layout_object` 에 점유 폭·깊이(칸 수) 추가를 검토. 없다면 현행대로 두고
"넓은 설비 옆 칸은 비운다"를 배치 관행으로 공유하는 것으로 갈음할 수 있다.

#### 참고 — rack3d 가 사양서에서 **뺀** 것 (FMS 데이터가 없어서)

제품 오너가 준 사양서는 LCD 에 실시간 온/습도(22.5°C / 50%RH), 압력계에 42 Bar,
상태 LED 정상 Green / 장애 Red 전환을 요구한다. **전부 넣지 않았다** — FMS 는 배치 오브젝트의
텔레메트리를 주지 않는다. ZONE 온습도 시리즈는 존 센서값이지 그 장비의 값이 아니라,
장비 화면에 띄우면 남의 값을 그 장비 값처럼 보이게 한다. LED 를 전부 초록으로 켜 두는 것도
"전부 정상"이라는 거짓말이다. 화면은 꺼진 유리, LED 는 무발광 렌즈로 두었다.

→ 이 객체들의 상태·계측값을 FMS 가 제공할 계획이 있으면 알려주면 좋겠다. 있으면 그때
발광·수치 표시를 붙인다. 없으면 지금 상태가 최종이다.

### 11-39. rack3d 배포 — 배치 오브젝트 3D 모델 `main-42782d6` (2026-09-03)

색 박스로만 그리던 ZONE 배치 오브젝트에 실사풍 3D 모델을 붙였다(제품 오너 지시).
근거·규약·남은 결정은 `docs/layout-object-modeling-plan.md`, FMS 협의 3건은 §11-38.

| 항목 | 값 |
|---|---|
| 이미지 | `10.1.20.21:5000/rack3d-web:main-42782d6` |
| 커밋 | `42782d64f187297215ae398a04dd2cb733607ef5` (서버에서 실측 대조) |
| 직전 운영본 | `main-c050820` |
| 롤아웃 | Deployment `rack3d-web` 리비전 12, 무중단 롤링, 재시작 0 |

**라이브에서 실제로 보이는 것까지 확인했다**(스크립트 성공 ≠ 배포 성공).
- 새 GLB 4개가 `?v=12` 로 200 (합 469,188 B). 바이트수만이 아니라 **sha256 을 git blob 과 대조**해 4개 전부 일치.
- `index.html` 이 참조하는 번들이 새것(`index-CGAanznJ.js`), 옛 번들은 404 — 스테일 레이어 없음.
- 미니파이된 번들 안에서 `MODEL_VERSION` 이 `12` 로 반영된 것을 확인.

**이번 배포로 화면에 새로 나오는 것은 `CRAC`·`UPS` 2종뿐이다.**
배터리 랙·가스 소화 설비 모델도 함께 나갔지만 **FMS 팔레트에 해당 type 이 없어** 아직 그려지지 않는다.
§11-38 ①②가 확정되면 **rack3d 를 고치지 않아도** 붙는다(`LayoutObject.type` 이 `string` 이라
모르는 값을 흘려보내는 계약 덕이다).
...
- POWER/SEISMIC/FIRE는 기존 FMS 타입. 사용자 확정(2026-09-11): 태깅기는 모델만 보관,
  기존 GATE 유지. 사용자가 추후 변경을 알리기 전에는 연결하지 않는다.
