...
- [ ] 2026-09-11 사용자 검수: **UPS 하나 먼저**, 나머지는 승인 후 일괄 제작
  - ✅ 최신 엑셀 `ups` 시트의 Standard / SD-ups 도면으로 기존 임시 UPS 교체.
    기준 800×1000×2000mm, 최종 GLB 외곽 800×998×2000mm(패널 여유 포함).
    전면 LCD·버튼·상하 그릴·좌측 손잡이, 측면 흡기, 상부 팬 4개, 후면 점검 패널.
  - ✅ `UPS` 타입 매핑 유지, 이름표 높이 2m 및 모델 캐시 버전 13 반영.
  - ✅ 자체 리뷰·Blender GLB 재수입 앞뒤 렌더·스텁 앱 화면 QA·빌드·린트 통과.
    GLB 6,412 tris / 5 primitives / 391,684 bytes / 이미지 0장. 앱의 `ups.glb?v=13` 수신 확인.
  - 사용자 테스트용 로컬 스텁: `http://localhost:5174/rack3d/` (동일 LAN: `http://192.168.0.43:5174/rack3d/`).
    운영 배포는 아직 하지 않음. 실제 FMS 데이터 검수·사용자 피드백 대기.
  - 후속: 온습도센서·배터리랙·가스소화설비·도어·누수감지.
    표준서버/표준네트워크는 랙 내부용 U별 형상 + 정확한 모델 미매칭 시 폴백으로 별도 진행.
    이번 UPS 작업에서는 기존 서버 선택 정책을 변경하지 않음.
...
- [ ] 🛠 배치 오브젝트 8종 실사풍 3D 모델 — 색 박스를 대체한다 (E18 ④ 결정 뒤집기)
  ...
  - ✅ 배포 절차 문서화 — `deploy/README.md` + `.claude/agents/deployer.md`(프로젝트 전용)
