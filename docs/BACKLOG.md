...
- [ ] 2026-09-11 배전반·지진감지·화재감지·출입태깅기
  ...
  - 🔵 사용자 후속 요청(2026-09-11): GATE 유지 + 출입태깅기 독립 배치.
    rack3d는 제안 타입 ACCESS → access-tag-reader 연결 준비, 높이 1.2m는 표현용.
    FMS 배치 팔레트/스키마에는 ACCESS가 없어 타입 합의·추가 및 실제 좌표 설정 필요.
    GATE 유지·독립 좌표·배치 없는 장비 생성 금지 회귀 검사 통과. 운영 미반영.
  - ✅ 4종 치수·렌더·브라우저 카탈로그, Node 7개·빌드·린트 검증.
  - ✅ 운영 배포 `main-0cd3ec5`: Ready 1/1, GLB 36개 해시·새 JS 로컬 빌드 일치, 이전 JS 404.
  - ✅ Vite(5174)·FMS 스텁(8777)·테스트 Chrome(9222) 종료 및 프로세스/리스너 없음 확인.
  - 운영 FMS 로그인 후 육안 검수는 사용자 확인 대기. 상세: 모델링 계획 §15 / 연동 문서 §11-42.

- [ ] 2026-09-11 CCTV 모델 운영 반영·사용자 검수
  - ✅ Google Sheets CCTV 시트 확인, Standard SD-CCTV GLB 제작 및 기존 CCTV 타입 연결.
  - ✅ 40×180×150mm, 1,880 triangles, 브래킷·케이블 포함. MODEL_VERSION=15.
  - ✅ Node 7개·빌드·린트·Blender 렌더·브라우저 카탈로그 확인.
  - 설치 높이 2.2m는 FMS 높이 필드가 없어 사용하는 표현용 가정.
  - ✅ 운영 배포 `main-faf7f43`: Ready 1/1, GLB 32개 SHA256 및 새 JS 로컬 빌드 일치, 이전 JS 404.
  - FMS 로그인 후 사용자 육안 검수 필요. 상세: 모델링 계획 §14, 보안·연동 문서 §11-41.

- [ ] 2026-09-11 표준 모델 운영 반영·사용자 검수
  - ✅ UPS 사용자 확인 후 후속 설비 5종(온습도·배터리랙·소화설비·도어·누수) 제작 및 연결 완료.
  - ✅ 표준서버·표준네트워크 **각 1~10U / 총 20종** 생성. 제조사+모델명으로 특정되면
    기존 벤더 GLB 유지. Standard/SD-u·SDN-u 명시 또는 모델 미특정 시에만 표준 형상 사용. 10U 초과는 10U 형상을 실제 U로 늘림.
  - ✅ 표준 모델도 FMS 실사진 적용. 사진 미수신/실패/수요 해제 시 중립 3D 형상 유지.
    기존의 "사진 없는 자산도 벤더 기본 사진 유지" 결정은 **정확히 매칭된 벤더 GLB에만** 적용된다.
  - ✅ 최신 시트 치수 반영: UPS/배터리랙 800×1000×2000mm, 소화 단일 용기·배관 2400mm,
    도어 900×2100mm 유리문+센서. 누수는 최신 도면대로 바닥형, 온습도는 실치수+1.2m 설치 표시.
  - ✅ 자체 리뷰, Node 테스트 5개, GLB 25종 재수입 렌더, 브라우저 카탈로그 26개 로딩,
    앱 앞뒤 실사진·거리 이탈 시 평면 숨김/텍스처 해제, 빌드·린트 검증. MODEL_VERSION=14.
  - ✅ 사용자 정정 반영: FMS 랙 소속·좌표·방향·시작U·점유U를 변경하지 않음.
    모델 선택에 추가했던 U 일치 제한 제거, Standard 코드 직접 인식 추가.
    특정 모델/표준 명시/미특정 3경로의 배치 보존 회귀 검사 및 빌드·린트 통과.
  - 검수: `http://192.168.0.43:5174/rack3d/scripts/dev/model-catalog.html` (같은 LAN, 개발 서버 필요).
    앱 스텁: `http://192.168.0.43:5174/rack3d/`. ✅ 운영 배포 완료 `main-e3d84f6` (2026-09-11): Ready 1/1, 라이브 GLB 31개 SHA256 일치,
    새 JS 번들 로컬 빌드와 일치, 이전 JS 404. 실제 FMS 로그인 후 육안 검수는 사용자 확인 대기.
  - ⬜ **FMS BATTERY/SUPPRESSION 타입 신설** 필요 — 현재 팔레트에 없음. 스텁/카탈로그에서 검수 가능.
    GAS는 가스감지기이므로 소화설비로 바꾸지 않는다. 기존 협의 근거는 §11-38.
  - ⬜ 다중칸 점유 협의: UPS/배터리랙/CRAC/도어는 600mm 타일을 넘어 이웃 배치와 겹칠 수 있다.
  - 화재감지(FIRE)는 후속 요청으로 모델 추가(모델링 계획 §15).
  - 제작·제약·재생성·검증 상세: `docs/layout-object-modeling-plan.md` §12~13.
